...
def fetch_data(ticker: str = "MSFT", start_date: str = "2019-06-01", end_date: str = "2024-06-01"):
    # Fetch data
...
def prepare_data(data: pd.Series, input_size: int, forecast_size: int):
    ...
    # Convert to numpy array and ensure float type for TimeSeries
...
def main(train_model=True):
   ...
   # Parameters
    ...
    if train_model:
        ...
        torch.save(model.state_dict(), "nbeats_MSFT.pth")
        ...
        model.load_state_dict(torch.load("nbeats_MSFT.pth"))
        model.eval()
